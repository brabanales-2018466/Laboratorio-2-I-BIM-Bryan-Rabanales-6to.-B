...
const asignarAlumno = async(req = request, res = response) =>{
    ...
    if(!existeCurso){
        return res.status(404).json({
            msg: 'Curso no encontrado para la asignación'

        })
    }
    if(cursos.length >= 3){
        return res.status(400).json({
            msg: 'Ya tiene el maximo de 3 cursos a los que esta asignado'
    ...
    if(exists) return res.status(400).json({ msg: 'Curso ya asignado'})
    ...
    res.status(200).json({
        msg: 'Asignación al curso de manera existosa',
        updatedUser,
        updatedCurse
    })
}
